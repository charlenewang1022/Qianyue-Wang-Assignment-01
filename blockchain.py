...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        mrh = block.get_simple_merkle_root()
        if not self.__prev_hash :
            self.__prev_hash = ''
            
        block_hash = hashlib.sha256((self.__prev_hash + self.__id + str(dt.datetime.now()) + str(self.__seq_id) + mrh).encode('utf-8')).hexdigest()# Create block hash
        # Add code here
    ...
    def get_values(self):
        ...
        for b in self.__chain:
            res += b.get_values()
        res += self.__current_block.get_values()
        return res
...
class Block:
    ...
    def display_header(self): 
        print('sequence Id:',self.__seq_id,
              ...
              '  number of transactions:',self.get_size())
    ...
    def add_transaction(self,s,r,v): 
        ...
        new_transaction = pd.DataFrame.from_dict({'Timestamp' : [ts], 
                                                  'Sender'    : [s],
                                                  'Receiver'  : [r],
                                                  'Value'     : [v],
                                                  'TxHash'    : [tx_hash]})# Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def get_simple_merkle_root(self): 
        all_hash = ""
        for h in self.__transactions['TxHash']:
            all_hash += h
        mrh = hashlib.sha256(all_hash.encode('utf-8')).hexdigest()
        self.__merkle_tx_hash = mrh
        return mrh
